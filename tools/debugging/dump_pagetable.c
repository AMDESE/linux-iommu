...
#define BOLD_OFF "\033[0m"
	...
	if (addr >> 32)
		sprintf(b, "%04llx", (addr & 0xFFFF0000) >> 16);
	else if (addr >> 16)
		if (align)
			sprintf(b, "%4llx", (addr & 0xFFFF0000) >> 16);
		else
			sprintf(b, "%llx", (addr & 0xFFFF0000) >> 16);
	else if (align)
		strcpy(b, "    ");
...
#define prrmpaddr(b, addr) __praddr((b), sizeof(b), 0, (addr))
...
static const char *rmp(char *b, u64 pfn)
{
	...
	sprintf(b, " / RMP#%d:%s %s asid%d %s%s%s%s%s%s%s%s%s%s",
		segno,
		prrmpaddr(ba, (u64)e.gfn << PAGE_SHIFT),
		e.page_size ? "2M" : "4K",
		e.guest_asid,
...
static int dump_ptes(u64 pfn, unsigned tabs, const char *pgsz, u64 addr, u64 *ptes)
{
	...
	char buf[128], ba[32], bo[32], bp[32];
	int ret, first, i;
...
#define FL(x) ((dte[i].x)?'+':'-')
		printf("%s: h=%llx mode=%d ir%c iw%c gv%c C%c\n",
			pci_name(tmp, i), ((u64)dte[i].host_ptr)<<PAGE_SHIFT, dte[i].mode,
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "l:d:f:p:q:n:r:R:s:S:t:T:e:vchP")) != -1) {
		switch (opt) {
		case 'f':
		...
		case 'd':
			...
			if (!g.dteoff)
				return pr_exit(opt, optarg, g.dteoff);
			break;
		case 'p':
		...
		case 'n':
			...
			nptpt = strtol(optarg, NULL, 16) >> PAGE_SHIFT;
			if (!nptpt)
		...
		case 'h':
		default: /* '?' */
			fprintf(stderr, "Usage: %s -f /dev/mem [-d dteaddr [-p BB:DD.Fn]] "
				"[-q qemuptaddr] [-n npt] [-l levels] [-r rmptable] [-R rmpcfg] "
	...
	if (g.fd < 0) {
		if (g.verbose > 0)
			printf("Fallback to /dev/mem\n");
		g.fd = open("/dev/mem", O_RDONLY | O_SYNC);
		if (g.fd < 0)
			return pr_exit('f', "/dev/mem", g.fd);
	}
	...
	else if (fn >= 8 && g.dteoff)
		ret = dump_dte();

	if (qemupt) {
		u64 ptes = 0;
		ret = dump_pdes(qemupt, lvl - 1, 0, 0, &ptes);
		if (g.verbose > 1)
	...
	if (nptpt) {
		...
		if (!g.ept)
			g.npt = 1;
		ret = dump_pdes(nptpt, lvl - 1, 0, 0, &ptes);
		if (g.verbose > 1)
			printf("PTES = %lld=%lldMB\n", ptes, ptes >> (20 - PAGE_SHIFT));
	}
...
#define min(a, b) ((a) < (b) ? (a) : (b))
	if (g.rmptable && !qemupt && !nptpt && fn >= 8)
