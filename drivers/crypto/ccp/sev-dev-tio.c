...
uint tiolog = TIO_LOG_SPDM | TIO_LOG_TIO | TIO_LOG_DOBJ | TIO_LOG_MEM | TIO_LOG_GUEST;
module_param_named(tiolog, tiolog, uint, 0660);
MODULE_PARM_DESC(tiolog, "Log level bitmask: 0 SPDM  1 TIOABI  3 DataObjects  4 STP  5 Guest  6 WA");
			  ...
			  struct tsm_dsm_tio *dev_data)
{
	...
	if (cmd == SEV_CMD_TIO_GUEST_REQUEST) {
		...
		sprintf(pfx + strlen(pfx), "%s(%x)#%llx ",
			guest_req_to_str(rq->msg_type), rq->msg_type, rq->msg_seqno);
	}
