...
static void pr_ide_states(struct pci_dev *pdev, struct pci_ide **ide)
{
	for (int i = 0; i < TIO_IDE_MAX_TC; ++i)
		if (ide[i])
			pr_ide_state(ide[i]);

	//sev_dump_sdte(0, 0, pci_dev_id(pdev));
}
...
static void tdi_unbind(struct pci_tdi *tdi)
{
	...
	if (!sev_tio_asid_fence_status(dev_data, device_id, segment_id,
		...
		if (fenced) {
			if (!tdi_data->unfence)
				pr_err("___K___ %s %u: Did not run, skip unfence\n", __func__, __LINE__);
			else {
...
static int tdi_run(struct tio_dsm *dsm, struct tio_tdi *ttdi)
{
	...
	if (tiolog & TIO_TDI_BIND) {
		pci_err(ttdi->tdi.pdev, "___K___ %s %u: Already running\n", __func__, __LINE__);
		return 0;
	}
			     ...
			     u64 *fw_err)
{
	...
	/*
	 * At the moment it is the only case when PCI_TSM_REQ_STATE_CHANGE
	 * and it means go to "RUN".
	 */
	if (scope == PCI_TSM_REQ_STATE_CHANGE) {
		u8 runstate = 0;

		if ((reqlen != sizeof(runstate)) || resplen)
			return -EINVAL;

		ret = copy_from_sockptr(&runstate, req, reqlen);
		if (ret)
			return ret;

		if (runstate == 1 && tdi->kvm)
			return tdi_run(dsm, ttdi);
		...
		/* Or resplen? It is expected to be zero here anyway though */
		return ret;
	}
