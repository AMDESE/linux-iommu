...
static void iommufd_vdevice_tsm_unbind(struct iommufd_vdevice *vdev)
{
	 ...
	 */
	WARN_ON_ONCE(domain->type != IOMMU_DOMAIN_UNMANAGED &&
...
int iommufd_vdevice_tsm_bind_ioctl(struct iommufd_ucmd *ucmd)
{
	...
	if (cmd->kvmfd == -1) {
		...
		rc = iommufd_vdevice_tsm_bind(vdev, to_pci_dev(idev->dev), cmd->kvmfd);
		if (rc)
			goto out_put_vdev;
	}
...
int iommufd_vdevice_tsm_guest_request_ioctl(struct iommufd_ucmd *ucmd)
{
	...
	domain = iommu_get_domain_for_dev(idev->dev);
	if (!domain) {
		rc = -ENODEV;
		goto out_put_viommu;
	}
	...
	switch (cmd->scope) {
	case IOMMU_VDEVICE_TSM_SCOPE_REQ_INFO:
		scope = PCI_TSM_REQ_INFO;
		break;
	case IOMMU_VDEVICE_TSM_SCOPE_REQ_STATE_CHANGE:
		scope = PCI_TSM_REQ_STATE_CHANGE;
		break;
	case IOMMU_VDEVICE_TSM_SCOPE_REQ_DEBUG_READ:
		scope = PCI_TSM_REQ_DEBUG_READ;
		break;
	case IOMMU_VDEVICE_TSM_SCOPE_REQ_DEBUG_WRITE:
		scope = PCI_TSM_REQ_DEBUG_WRITE;
		break;
	default:
		rc = -EINVAL;
		goto out_put_vdev;
	...
	// FIXME: have a "enable IOMMU" flag or something vs. counting that
	// tsm_enable() checkes if it is already enabled
	rc = domain->ops->tsm_enable(domain, idev->dev);
	if (rc)
		goto out_put_vdev;

	rsp_len = pci_tsm_guest_req(to_pci_dev(idev->dev), scope,
