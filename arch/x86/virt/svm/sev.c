...
int rmp_make_private_mmio(u64 pfn, u64 gpa, u32 asid)
{
       struct rmp_state state;
       ...
       state.asid = asid;
       /*
	* In order to avoid race between the PSP and RMPUPDATE, the PSP requires
	* MMIO RMPs to be made immutable during the MMIO_VALIDATE operation.
	* To make the MMIO accessible by the VM, the immutable bit needs to be
	* cleared, rmp_mmio_reclaim() below does that.
	*/
       state.immutable = true;
       state.gpa = gpa;
       /* No MMIO huge pages yet */
...
int rmp_make_shared_mmio(u64 pfn)
{
       struct rmp_state state;
